// PushSubscribe.jsx (prod-ready UI: hover states, loading, toasts, permission handling)
...
export default function PushSubscribe({ vapidPublicKey }) {
  ...
  // load saved watchlist and optionally resync if user previously enabled
  useEffect(() => {
    try {
    ...
    (async () => {
      try {
        ...
        // user explicitly turned off -> respect it
        if (userEnabled === "0") {
          setSubscribed(false);
          setShowEditor(false);
        ...
        // user previously enabled -> try to resync if permission granted
        if (userEnabled === "1") {
          if (perm === "granted") {
            if (sub) {
              try {
            ...
            // permission not granted -> wait for user to click
            setSubscribed(false);
            setShowEditor(false);
  ...
  // open chrome notification settings (helpful for users on desktop Chrome)
  ...
  // ENABLE: main user action, shows permission prompt (if allowed) and subscribes
  async function handleAllowClick() {
    ...
    // If permission is denied, the browser will not show prompt — inform user and guide to settings
    if (permissionState === "denied" || Notification.permission === "denied") {
      showToast("Notifications blocked by browser — open settings to re-enable", "error");
      return;
    }

    // prevent double clicks while enabling
    ...
    // permission granted — ensure SW registered and subscribe
    ...
    try {
      ...
      await fetch(`${API_BASE}/subscribe`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subscription: sub, criteria }),
      }).catch(() => { /* ignore network fails here */ });
  ...
  // add item (uncontrolled input) — keep focus for fast entry
  function addItem() {
  ...
  // save watchlist (send criteria to server)
    ...
    try {
      if (registration) {
        try {
          ...
          if (sub) {
            try { await sub.unsubscribe(); } catch (e) { /* ignore unsubscribe error */ }
          }
        } catch (e) { /* ignore */ }
      }
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div>
            {!showEditor ? (
              ...
              >
                {loadingEnable ? <span className="inline-flex items-center gap-2"><Spinner size={16} /> Enabling...</span> : "Enable"}
              </button>
            ) : (
            ...
            <div>
              ...
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${palette.border}`, color: palette.text }}
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addItem(); } }}
                  aria-label="Item to watch"
                />
                <button type="button" onClick={addItem} className="px-3 py-2 rounded-md text-[#071428] transition transform hover:-translate-y-0.5 active:scale-95" style={{ background: palette.primary }}>
                  Add
                </button>
              </div>
